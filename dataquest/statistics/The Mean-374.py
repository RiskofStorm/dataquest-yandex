...
above = [i - mean for i in distribution if i > mean]
below = [mean - i for i in distribution if i < mean]
...
for i in range(5000):
    seed(i)
    dist = randint(0,1000,10)
    mean = sum(dist)/len(dist)
    above = list()
    below = list()
    above = round(sum([i - mean for i in distribution if i > mean]),1)
    below = round(sum([mean - i   for i in distribution if i < mean]),1)
...
function_mean = mean(houses['SalePrice'])
pandas_mean = mean(houses['SalePrice'])
