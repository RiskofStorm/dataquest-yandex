...
for i in range(1,50,2):
    ...
    score = cross_val_score(knn, all_X, all_y, cv=10)
    accuracy_knn = score.mean()
    knn_scores[i] = accuracy_knn
...
score = cross_val_score(model, all_X,all_y,cv=10)
accuracy_rf = score.mean()
...
model = RandomForestClassifier(random_state=1)
grid= GridSearchCV(model, hyperparameters,cv=10)
grid.fit(all_X,all_y)
best_params = grid.best_params_
best_score = grid.best_score_
